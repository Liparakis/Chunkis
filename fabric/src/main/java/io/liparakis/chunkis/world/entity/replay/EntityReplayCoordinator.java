...
public final class EntityReplayCoordinator {
    ...
    public static ChunkRestorer.ReplayResult replayPendingEntityIfNeeded(
        ...
        NbtCompound entityNbt = runtimeDelta == null ? null : ChunkEntityQueries.findPendingEntityNbt(runtimeDelta, entityUuid);
        if (entityNbt == null) {
    ...
    private static SpawnOutcome attemptEntitySpawn(
        ...
        if (ChunkEntityQueries.isMatchingLiveEntity(existing, entity.getType(), chunkPosition)
                && ChunkEntityQueries.isVisibleFromWorldQuery(
                world,
    ...
    private static void traceEntityReplayDecision(
        ...
        final boolean visibleByQuery =
                uuid != null && ChunkEntityQueries.isVisibleFromWorldQuery(world, uuid, searchBox);
