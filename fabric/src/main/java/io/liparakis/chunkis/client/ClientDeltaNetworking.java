...
public final class ClientDeltaNetworking {
    ...
    private static void handleIncomingPayload(final ChunkDeltaPayload payload,
        ...
        if (isInvalidPayload(data)) {
            ChunkTraceStore.trace(ChunkisDebugDomain.CLIENT_SYNC, ChunkTraceEventType.CLIENT_SYNC_FAILED,
                    ChunkTraceSeverity.WARN, ChunkTraceReason.INVALID_PAYLOAD, APPLY_SOURCE, "rejected incoming " +
                            "payload because it was null or empty", null, new DebugChunkKey(payload.chunkX(),
                            payload.chunkZ()), null, operationId, null, data == null ? null : data.length);
    ...
    private static void processChunkDelta(final ChunkDeltaPayload payload, final ClientWorld world,
        ...
        if (world == null) {
            ChunkTraceStore.trace(ChunkisDebugDomain.CLIENT_SYNC, ChunkTraceEventType.CLIENT_SYNC_FAILED,
                    ChunkTraceSeverity.INFO, ChunkTraceReason.CLIENT_WORLD_UNAVAILABLE, APPLY_SOURCE, "skipped " +
                            "payload because client world was unavailable", null, chunkKey, null, operationId, null,
                    payload.data().length);
            return;
        }

        final long startNanos = captureStartTime();
        ChunkTraceStore.trace(ChunkisDebugDomain.CLIENT_SYNC, ChunkTraceEventType.CLIENT_SYNC_TX_START,
                ChunkTraceSeverity.INFO, ChunkTraceReason.NONE, APPLY_SOURCE, describeIncomingPayload(payload,
                        "starting client delta apply"), world.getRegistryKey().getValue().toString(), chunkKey, null,
                operationId, null, payload.data().length);
        ...
        try {
            ...
            ChunkTraceStore.trace(ChunkisDebugDomain.CLIENT_SYNC, ChunkTraceEventType.CLIENT_SYNC_TX_END,
                    ChunkTraceSeverity.INFO, ChunkTraceReason.NONE, APPLY_SOURCE, describeIncomingPayload(payload,
                            "completed client delta apply"), world.getRegistryKey().getValue().toString(), chunkKey,
                    null, operationId, null, payload.data().length);

            Chunkis.LOGGER.debug("Applied ChunkDelta ({},{}) - {} bytes, {} blocks", payload.chunkX(),
                    payload.chunkZ(), payload.data().length, changedBlockCount);

        } catch (final IOException e) {
            ChunkTraceStore.trace(ChunkisDebugDomain.CLIENT_SYNC, ChunkTraceEventType.CLIENT_SYNC_FAILED,
                    ChunkTraceSeverity.ERROR, classifyClientSyncFailure(e), APPLY_SOURCE, "client delta decode " +
                            "failed", world.getRegistryKey().getValue().toString(), chunkKey, null, operationId, null
                    , payload.data().length);
            ClientDeltaMetrics.logErrorThrottled(() -> String.format("Decode failed for chunk (%d,%d) - %d bytes",
                    payload.chunkX(), payload.chunkZ(), payload.data().length), e);
        } catch (final Exception e) {
            ChunkTraceStore.trace(ChunkisDebugDomain.CLIENT_SYNC, ChunkTraceEventType.CLIENT_SYNC_FAILED,
                    ChunkTraceSeverity.ERROR, ChunkTraceReason.IO_EXCEPTION, APPLY_SOURCE, "client delta apply " +
                            "failed" + " with exception", world.getRegistryKey().getValue().toString(), chunkKey,
                    null, operationId, null, payload.data().length);
    ...
    private static int applyPayloadToWorld(final ChunkDeltaPayload payload, final ClientWorld world,
        ...
        if (!isChunkisDuck(chunk, chunkX, chunkZ)) {
            ChunkTraceStore.trace(ChunkisDebugDomain.CLIENT_SYNC, ChunkTraceEventType.CLIENT_SYNC_FAILED,
                    ChunkTraceSeverity.WARN, ChunkTraceReason.CHUNK_NOT_DELTA_CAPABLE, APPLY_SOURCE,
                    "client chunk " + "did not implement ChunkisDeltaDuck",
                    world.getRegistryKey().getValue().toString(), new DebugChunkKey(chunkX, chunkZ), null,
                    operationId, null, payload.data().length);
            return 0;
        }
        ...
        @SuppressWarnings("unchecked") final ChunkDelta<BlockState, NbtCompound> clientDelta = (ChunkDelta<BlockState
                , NbtCompound>) ((ChunkisDeltaDuck) chunk).chunkis$getDelta();

        applyDelta(clientDelta, receivedDelta, world, chunkX, chunkZ);
        PayloadWatchTracer.traceLiveChunkState(chunk, ChunkTraceEventType.WATCH_PRESENT_IN_CLIENT_WORLD, "client" +
                "-world", APPLY_SOURCE, operationId, receivedDelta);
        return receivedDelta.getBlockInstructions().size();
    }

    private static void applyDelta(final ChunkDelta<BlockState, NbtCompound> clientDelta, final ChunkDelta<BlockState
            , NbtCompound> receivedDelta, final ClientWorld world, final int chunkX, final int chunkZ) {
    ...
    private static void logTypeWarningOnce(final WorldChunk chunk, final int chunkX, final int chunkZ) {
        ...
        Chunkis.LOGGER.warn("Chunkis: Chunk at ({},{}) does not implement ChunkisDeltaDuck: {}. (Warning shown once " + "only.)", chunkX, chunkZ, chunk.getClass().getName());
    }
