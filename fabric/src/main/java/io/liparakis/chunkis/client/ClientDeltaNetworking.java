...
public final class ClientDeltaNetworking {
    ...
    private static void handleIncomingPayload(final ChunkDeltaPayload payload,
        ...
        if (isInvalidPayload(data)) {
            ChunkTraceStore.trace(ChunkisDebugDomain.CLIENT_SYNC, ChunkTraceEventType.CLIENT_SYNC_FAILED,
                                  ChunkTraceSeverity.WARN, ChunkTraceReason.INVALID_PAYLOAD, APPLY_SOURCE,
                                  "rejected incoming " +
                                          "payload because it was null or empty", null, new DebugChunkKey(payload.chunkX(),
                                                                                                          payload.chunkZ()), null, operationId, null,
    ...
    private static void processChunkDelta(final ChunkDeltaPayload payload, final ClientWorld world,
        ...
        if (world == null) {
            ChunkTraceStore.trace(ChunkisDebugDomain.CLIENT_SYNC, ChunkTraceEventType.CLIENT_SYNC_FAILED,
                                  ChunkTraceSeverity.INFO, ChunkTraceReason.CLIENT_WORLD_UNAVAILABLE, APPLY_SOURCE,
                                  "skipped " +
        ...
        try {
            ...
            ChunkTraceStore.trace(ChunkisDebugDomain.CLIENT_SYNC, ChunkTraceEventType.CLIENT_SYNC_FAILED,
                                  ChunkTraceSeverity.ERROR, classifyClientSyncFailure(e), APPLY_SOURCE,
                                  "client delta decode " +
            ...
            ChunkTraceStore.trace(ChunkisDebugDomain.CLIENT_SYNC, ChunkTraceEventType.CLIENT_SYNC_FAILED,
                                  ChunkTraceSeverity.ERROR, ChunkTraceReason.IO_EXCEPTION, APPLY_SOURCE,
                                  "client delta apply " +
                                          "failed"
                                          + " with exception", world.getRegistryKey().getValue().toString(), chunkKey,
                                  null, operationId, null, payload.data().length);
    ...
    private static void logTypeWarningOnce(final WorldChunk chunk, final int chunkX, final int chunkZ) {
        ...
        Chunkis.LOGGER.warn("Chunkis: Chunk at ({},{}) does not implement ChunkisDeltaDuck: {}. (Warning shown once "
                                    + "only.)", chunkX, chunkZ, chunk.getClass().getName());
    }
