...
final class ChunkDebugOutput {
    ...
    static String formatEvent(final ChunkTraceEvent event, final DateTimeFormatter timeFormat) {
        ...
        if (event.dirtyState() != null) {
            sb.append(" dirty=").append(event.dirtyState());
        }
        if (event.byteSize() != null) {
            sb.append(" bytes=").append(event.byteSize());
        }
        if (event.operationId() != null) {
            sb.append(" op=").append(event.operationId());
        }
        if (event.payloadWatchTarget() != null) {
            sb.append(" payload=").append(event.payloadWatchTarget().describe());
        }
        if (event.payloadWatchStage() != null) {
            sb.append(" stage=").append(event.payloadWatchStage());
        }
        if (event.payloadWatchSummary() != null) {
            sb.append(" payloadSummary=").append(event.payloadWatchSummary());
        }
    ...
    static String formatWatchpointSummary() {
        ...
        if (!chunks.isEmpty()) {
            sb.append(" chunks=").append(formatChunks(chunks));
        }
        if (!regions.isEmpty()) {
            sb.append(" regions=").append(formatRegions(regions));
        }
        if (!payloads.isEmpty()) {
            sb.append(" payloads=").append(formatPayloads(payloads));
        }
        return sb.toString();
    }
