...
| `core/src/main/java/io/liparakis/chunkis/debug/ChunkTraceSuspect.java` | `SUPPORTING_HOOK_REQUIRED` | `PHASE_3_PARTIAL` | Retained suspect snapshot record with stable suspect id, original failure event copy, and copied compact timeline that survives main-ring rotation. |
...
| `core/src/main/java/io/liparakis/chunkis/debug/ChunkTraceStore.java` | `CORE_HOOK_REQUIRED` | `PHASE_3_PARTIAL` | Bounded in-memory event store with monotonic ids, near-zero OFF-path cost, watched-event filtering, chronological snapshot queries for export, store-boundary `ASSERTION_FAILED` emission for malformed high-value events, and a separately bounded retained suspect registry with copied chunk/op timelines. |
...
| `core/src/main/java/io/liparakis/chunkis/storage/io/CisStorage.java` | `CORE_HOOK_REQUIRED` | `PHASE_3_PARTIAL` | Save/load start-end, flush lifecycle, storage-boundary decode-failure classification, and optional `PARANOID` raw write read-back verification are now traced. |
...
| `fabric/src/main/java/io/liparakis/chunkis/client/ClientDeltaNetworking.java` | `CORE_HOOK_REQUIRED` | `PHASE_2_DONE` | Client receive/apply start-end and top-level failure paths now emit structured sync evidence, including malformed-payload decode classification and wire-compression state. |
...
| `fabric/src/main/java/io/liparakis/chunkis/command/ChunkDebugCommand.java` | `SUPPORTING_HOOK_REQUIRED` | `PHASE_3_PARTIAL` | `/chunkis debug` now covers chunk/region watchpoints, watched-event queries, watched pending-save snapshots, world-local JSONL export, and retained suspicion-driven suspect/failure discovery via stable suspect ids rather than list position. |
...
| `fabric/src/main/java/io/liparakis/chunkis/network/ChunkisNetworking.java` | `CORE_HOOK_REQUIRED` | `PHASE_2_DONE` | Server-side delta send start-end and top-level drop/failure paths now emit structured sync evidence, including compression-state summaries. |
...
| `fabric/src/main/java/io/liparakis/chunkis/storage/AsyncCisSaveManager.java` | `CORE_HOOK_REQUIRED` | `PHASE_3_PARTIAL` | Async queue acceptance and worker failure path now emit structured evidence, and the pending queue can now be snapshotted for watched chunks. |
| `fabric/src/main/java/io/liparakis/chunkis/storage/BaseChunkCaptureScheduler.java` | `CORE_HOOK_REQUIRED` | `PHASE_3_PARTIAL` | Deferred/save-flush rejection path now emits structured evidence, and the queued base-capture set can now be snapshotted for watched chunks. |
...
| --- | --- | --- | --- |
| `core/src/test/java/io/liparakis/chunkis/debug/ChunkTraceStoreTest.java` | `DEBUG_VALIDATION_SUPPORT` | Debug defaults OFF, monotonic bounded store behavior, newest-event retention, chronological snapshot order, store-boundary assertion emission for malformed save events, suspicion promotion from stored-payload/load-miss plus dirty-unload timelines, and retained suspect snapshot survival after main-ring rotation. | Extend later only if export begins tracking additional metadata, pagination, or broader malformed event families. |
...
| `core/src/test/java/io/liparakis/chunkis/storage/io/CisStorageTraceTest.java` | `DEBUG_VALIDATION_SUPPORT` | Save/load storage hooks emit flush and region read/write transaction events, and `PARANOID` writes perform verification read-back without false assertions on the happy path. | Extend later to assert deliberate mismatch/failure paths once a low-churn corruption fixture is worth adding. |
...
| `fabric/src/test/java/io/liparakis/chunkis/command/ChunkDebugCommandTest.java` | `DEBUG_VALIDATION_SUPPORT` | Timeline formatter, stable-id suspect summary/detail formatting, watchpoint summary formatting, and watched pending-snapshot formatting for command output. | Extend later to assert suspect/failure command execution paths once a low-churn command harness is worth the churn. |
...
Top-level receive/apply start-end and failure events are implemented. Client malformed-payload failures now distinguish `DECODE_FAILED` vs `MAPPING_LOOKUP_FAILED`, the apply path no longer re-decodes just for metrics/logging, and the send/apply messages now expose compression state. Future work is only shared cross-wire ordering/correlation if a concrete timeline still proves ambiguous.
...
- first dirty mutation origin propagation
...
Send start-end, empty-delta/player-unavailable skips, payload-too-large drops, send failures, and compression-state summaries are implemented. Future work is shared cross-wire correlation only if the current timeline still proves ambiguous.
...
- Current behavior:
  validates authoritative-delta replacement rules, chunk-unload trace emission, and live-mutation origin propagation.
- Later observability assertions:
...
## `fabric/src/test/java/io/liparakis/chunkis/mixin/world/WorldChunkMixinTest.java`

- Current behavior:
  validates that a proto-carried restore operation id is consumed and reused by the live restore boundary.
- Later observability assertions:
  restore should keep the same operation id through proto attach and live replay in broader integration tests.
- Missing tests to add:
  one focused test for fallback restore-id generation only if the no-id branch becomes meaningful.
