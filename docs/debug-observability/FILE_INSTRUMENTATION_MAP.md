...
| `fabric/src/main/java/io/liparakis/chunkis/command/ChunkDebugCommand.java` | `SUPPORTING_HOOK_REQUIRED` | `PHASE_3_PARTIAL` | Minimal `/chunkis debug` command surface now includes chunk/region watchpoints, watched-event queries, watched pending-save snapshots, and world-local JSONL export. |
...
| `fabric/src/test/java/io/liparakis/chunkis/command/ChunkDebugCommandTest.java` | `DEBUG_VALIDATION_SUPPORT` | Timeline formatter, watchpoint summary formatting, and watched pending-snapshot formatting for command output. | Extend later to assert export and pending command execution paths once command harness coverage is worth the churn. |
